...
$(document).ready(function () {
   ...
   // $.get("module/photosbackend/controller/controller_photosbackend.class.php?load_data=true",
    $.get("../../photosbackend/load_data_photos/",{'load_data':true},
        function (response) {
            alert(response.user);
            if (response.user === "") {
                $("#link").val('');
    ...
    //Dropzone function //////////////////////////////////
    $("#dropzone").dropzone({
      //  url: "module/photosbackend/controller/controller_photosbackend.class.php?upload=true",
        url: "../../photosbackend/upload_photos/",
        params:{'upload':true},
        addRemoveLinks: true,
        maxFileSize: 1000,
        ...
        removedfile: function (file, serverFileName) {
            ...
            $.ajax({
                type: "POST",
                //url: "module/photosbackend/controller/controller_photosbackend.class.php?delete=true",
                url: "../../photosbackend/delete_photos/",
...
function validate_photos() {
    ...
    if (result) {
        ...
        $.post('../../photosbackend/alta_photos/', {alta_photos_json: data_photos_JSON},
            function (response) {
            ...
            if (xhr.responseJSON.error.link)
                $("#link").focus().after("<span  class='error1'>" + xhr.responseText.error.link + "</span>");
            ...
            if (xhr.responseJSON.error.descr){
                ...
                alert(xhr.responseJSON.error.descr);
                $("#descr").focus().after("<span  class='error1'>" + xhr.responseText.error.descr + "</span>");
            }
            ...
            if (xhr.responseJSON.success1) {
                if (xhr.responseJSON.img_avatar !== "/html5up-arcana/media/default-avatar.png") {
                    //$("#progress").show();
                    //$("#bar").width('100%');
                    //$("#percent").html('100%');
                    //$('.msg').text('').removeClass('msg_error');
...
function load_countries_v2() {
    ...
    $.post("../../photosbackend/load_pais_photos/",{'load_pais':true}, 
        function(data) {
...
function load_countries_v1() {
  //var c = amigable_js("?module=photosbackend&function=load_pais");
    //console.log(c);
   // $.post(c,
   $.post("../../photosbackend/load_pais_photos/",{'load_pais':true},
        function( response ) {
            ...
            if(response === 'error'){
                ...
                load_countries_v2(); //oorsprong.org
            }
    })
...
function load_provinces_v1() { //provinciasypoblaciones.xml - xpath
    $.post( "../../photosbackend/load_provincia_photos/", {'load_provincia':true},
        function( response ) {
          $("#provincia").empty();
	        $("#provincia").append('<option value="" selected="selected">Selecciona provincia</option>');
...
function amigable_js(url) {
    ...
    var SITEROOT = "127.0.0.1/html5up-arcana"
